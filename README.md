...
| Ngày giao hàng DO | Một ngày duy nhất cho toàn bộ DO (không theo từng dòng SP). Mặc định = ngày mong muốn từ PO (`date_planned`). NCC có thể đổi → đây là ngày giao thực tế → `scheduled_date` trên `stock.picking` |
| Chỉnh sửa DO | Vendor chỉnh sửa ngày giao hàng + số lượng (qty <= qty đặt hàng từ PO) + ghi chú inline |
...
| Vendor từ chối RFQ | Vendor từ chối Sent RFQ qua portal → trạng thái Odoo PO chuyển sang Canceled + email gửi PO creator |
...
- Vendor nhấp "Từ chối" → trạng thái Odoo PO chuyển sang Canceled
...
**Xem chi tiết PO:** nhấp vào PO hiển thị toàn bộ danh sách sản phẩm đặt hàng với số lượng và ngày giao hàng dự kiến, cùng với DO liên kết và trạng thái của nó (Draft / Signed / Done / Canceled).
...
flowchart TB
    ...
    subgraph R2["② Delivery Order — Chỉnh sửa & In"]
        direction LR
        P_DO["Odoo tự tạo DO\nPortal đồng bộ"]:::portal
        V1(["Vendor chỉnh sửa DO\nSố lượng + Ghi chú inline"]):::vendor
        P1["In DO PDF\n(nhiều lần, không cần ký)"]:::portal
        P_DO --> V1 --> P1
    end
...
#### 3.2 Delivery Order — Chỉnh sửa & In
...
- Vendor chỉnh sửa: **ngày giao hàng** + số lượng từng dòng (không vượt qty đặt hàng) + **ghi chú inline trên từng dòng sản phẩm** (giảm thao tác so với phải bấm In → điền ghi chú)
- Ngày giao trên DO cập nhật vào `scheduled_date` trên `stock.picking` — **không cập nhật ngược vào PO** nên không conflict (ngày trên PO là ngày dự kiến ban đầu, không đổi)
- **Không cần ký điện tử** — vendor in DO PDF trực tiếp từ giao diện, không qua bước ký
...
- Lúc **23:00 đêm trước ngày giao hàng trên DO**, DO tự động chuyển sang trạng thái **Locked**
- Ngày giao trên DO mặc định = ngày mong muốn giao hàng từ PO (`date_planned`). NCC có thể đổi ngày giao → **cutoff dời theo**
- Ví dụ: NCC đổi ngày giao từ 15/4 → 20/4 thì cutoff chạy lúc 23:00 ngày 19/4
...
**Nút hành động:**
- Đổi tên "Xác nhận Đơn Đặt Hàng" → **"Xác Nhận"** (ngắn gọn hơn)
- Sau khi vendor nhấn **"Xác Nhận"** → hiển thị nút liên kết đến phiếu giao hàng (DO)
...
| Số lượng giao | `stock.move.line.quantity_done` | Editable — vendor chỉnh |
...
**Ngày giao hàng trên DO (đã xác nhận):**
- NCC xác nhận PO ngay khi nhận — ngày trên PO là ngày dự kiến, không thay đổi
- Sau khi DO sinh ra, NCC có thể chỉnh ngày giao thực tế trên DO
- Ngày giao DO → `scheduled_date` trên `stock.picking` (không cập nhật ngược PO) → **không conflict**
...
| 3 | **Ngày giao hàng trên DO vs PO** | DO cho phép vendor chỉnh ngày giao hàng, PO đã có ngày dự kiến | ✅ **Đã xác nhận: Không conflict.** NCC xác nhận PO ngay khi nhận (ngày trên PO là ngày dự kiến, không đổi). DO sinh ra sau, NCC chỉnh ngày giao + qty trên DO. Ngày giao DO cập nhật vào `scheduled_date` trên `stock.picking`, không cập nhật ngược vào PO |
